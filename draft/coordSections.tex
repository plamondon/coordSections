...
The theory of cluster algebras gives rise to several interpretations of associahedra~\mbox{\cite{Tamari, Stasheff}}. \fref{fig:associahedron} shows two such interpretations for the rank~$3$ associahedron: as the exchange graph of triangulations of a hexagon and as the exchange graph of support $\tau$-tilting modules over the cluster tilted algebra whose quiver with relations is as depicted. 
This follows from results in the setting of the ``additive categorification of cluster algebras'' that was initiated in part in \cite{CalderoChapotonSchiffler, BuanMarshReinekeReitenTodorov}.
...
We will deduce this result from a general algebraic observation on support $\tau$-tilting modules and their $\b{g}$-vectors (see Section~\ref{sec:recollections} for definitions).
...
\]
then its $\b{g}$-vector is $\b{g}(P) = (a_i - b_i)_{i \in [n]}$.
...
In other words, the faces of $\siltingComplex(\Lambda)$ correspond to basic rigid objects whose $\b{g}$-vectors have zero coefficients in entries 
corresponding to elements in $J$.
...
As mentioned in the introduction, the $\dissection_\circ$-accordion complex is isomorphic to the $2$-term silting complex of the quiver~$\overline{\quiver}(\dissection_\circ)$ of the triangulation~$\dissection_\circ$.
...
Let~$\overline{\quiver} = (\quiver, I)$ be any gentle quiver with relations~\cite{ButlerRingel} and~$J$ be any subset of vertices of~$\overline{\quiver}$.
We call \defn{shortcut quiver} the quiver with relations~$\overline{\quiver}_J = (\quiver_J, I_J)$ whose vertices are the elements of~$J$, whose arrows are the paths in~$\overline{\quiver}$ with endpoints in~$J$ but no internal vertex in~$J$, and whose relations are inherited from those of~$\overline{\quiver}$.
Then the algebra~$k\quiver_J/I_J$ is gentle and is isomorphic to the algebra~$e_J (k\quiver/I) \, e_J$.
